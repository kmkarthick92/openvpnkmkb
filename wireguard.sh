...
PostUp = iptables -t nat -A PREROUTING -p tcp -i ${SERVER_NIC} --match multiport --dports 80,443,8443,${SERVER_PORT} -j DNAT --to-destination 10.66.66.2
PostUp = iptables -t nat -A POSTROUTING -o ${SERVER_NIC} -j SNAT --to-source ${SERVER_PUB_IP}
PostDown = iptables -t nat -D PREROUTING -p tcp -i ${SERVER_NIC} --match multiport --dports 80,443,8443,${SERVER_PORT} -j DNAT --to-destination 10.66.66.2
PostDown = iptables -t nat -D POSTROUTING -o ${SERVER_NIC} -j SNAT --to-source ${SERVER_PUB_IP}" >>"/etc/wireguard/${SERVER_WG_NIC}.conf"
	fi
